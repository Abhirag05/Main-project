...
interface VerifyInstallmentModalProps {
  isOpen: boolean;
  studentName: string;
  onConfirm: () => void;
  onCancel: () => void;
...
export default function VerifyInstallmentModal({
  isOpen,
  studentName,
  onConfirm,
  onCancel,
  isLoading,
}: VerifyInstallmentModalProps) {
        ...
        <div className="relative inline-block align-bottom bg-card rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full z-50">
              ...
              <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div className="mt-2">
                  <p className="text-sm text-muted-foreground">
                    Are you sure you want to verify installment payment for{" "}
                  ...
                  <div className="mt-3 p-3 bg-primary/10 border border-primary/20 rounded-md">
                    ...
                    <ul className="mt-2 text-sm text-primary list-disc list-inside space-y-1">
                      ...
                      <li>Mark payment status as installment</li>
                      <li>Require monitoring for future payments</li>
                    </ul>
                  </div>
                  <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                    <p className="text-xs text-yellow-700">
                      <strong>Note:</strong> You can disable access later if installments are not paid on time.
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-secondary/50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            <button
              ...
              onClick={onConfirm}
              disabled={isLoading}
              className="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-primary text-base font-medium text-white hover:bg-primary/90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-ring sm:ml-3 sm:w-auto sm:text-sm disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? "Verifying..." : "Verify Installment"}
            </button>
            <button
              type="button"
              onClick={onCancel}
