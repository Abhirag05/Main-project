...
interface SetPendingModalProps {
  isOpen: boolean;
  studentName: string;
...
export default function SetPendingModal({
  isOpen,
  studentName,
  currentStatus,
  onConfirm,
        ...
        <div className="relative transform overflow-hidden rounded-lg bg-card text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
          ...
          <div className="bg-secondary/50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
            <button
              ...
              onClick={onConfirm}
              disabled={isLoading}
              className="inline-flex w-full justify-center rounded-md bg-yellow-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-yellow-700 sm:ml-3 sm:w-auto disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
                <>
                  <svg
